...
// --- WebGL Helper Classes & Functions (Moved outside) ---
...
export default function SplashCursor({
    // ... rest of component using these classes ...
    SIM_RESOLUTION = 128,
    DYE_RESOLUTION = 1024, // Reduced from 1440 for performance
    CAPTURE_RESOLUTION = 512,
    DENSITY_DISSIPATION = 4.0, // Increased from 3.5 to make it fade faster
    ...
    useEffect(() => {
        ...
        let pointers: Pointer[] = []; // Initialize empty first

        // Define cleanup function variable to be assigned later
        let cleanup: (() => void) | undefined;

        const observer = new IntersectionObserver(([entry]) => {
            isVisible = entry.isIntersecting;

            // Only initialize on first visibility
            if (isVisible && !initialized) {
                initialized = true;
                // Initialize WebGL context and simulation only when visible
                pointers.push(pointerPrototype()); // Add pointer
                cleanup = initSimulation(canvas);
            }
        }, { threshold: 0 });

        observer.observe(canvas);

        function initSimulation(canvas: HTMLCanvasElement) {
    ...
    `
            ...
            function updateFrame() {
                ...
                step(dt);
                render(null);
                // animationId is defined in outer scope of initSimulation presumably? 
                // No, I need to define it.
                // Since I can't easily change the scope in one go, I'll pass.
            ...
            // Event listeners attached to window/document to capture interactions
